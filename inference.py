...
class ChatRequest(BaseModel):
    ...
    pii_output_threshold: float = Field(default=DEFAULT_PII_THRESHOLD, ge=0.0, le=1.0)
...
def chat(req: ChatRequest) -> ChatResponse:
    ...
    if STEP_PROMPT_GUARD not in req.disabled_steps:
        prompt_guard = gateway.run_prompt_guard(req.prompt, req.prompt_injection_threshold)
    ...
    if state["allowed"] and STEP_HARMFUL_GUARD not in req.disabled_steps:
        harmful_guard = gateway.run_harmful_guard(req.prompt, req.harmful_content_threshold)
    ...
    if (
        ...
        pii_output_guard = gateway.run_pii_output_guard(state["response"], req.pii_output_threshold)
